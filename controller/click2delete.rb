class Click2delete < Controller
	...
	def index
		...
		output << "<p>Click2delete items work with tables, but can look visually different depending on your browser. The effect will always look different to using a block element like &lt;div&gt; as the same effect as in <strong>example 2</strong> below does not work.</p>"
		...
		rows = (1..10)
		rows.each do |i|
			rwb = "<td>Row " + i.to_s + "!</td><td>Some more text to pad it out a bit</td><td>" + click2delete_button({}, '/click2delete/ajax') + "</td>"
	...
	rows.each do |i|
		...
		output << "<p>This example uses the default &lt;div&gt; element for the wrapper and looks far better in all browsers. The controller will refuse to delete rows 4 and 5</p>"
